...
def decrypt_data(encrypted_text, passkey):
    global failed_attempts  # Declare global variable at the start of the function
    hashed_passkey = hash_passkey(passkey)

    for key, value in stored_data.items():
        if value["encrypted_text"] == encrypted_text and value["passkey"] == hashed_passkey:
            failed_attempts = 0  # Reset failed attempts on successful decryption
            return cipher.decrypt(encrypted_text.encode()).decode()

    failed_attempts += 1  # Increment failed attempts if decryption fails
    return None
